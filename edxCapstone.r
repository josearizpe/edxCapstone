...
# FIND OUT ALL DIFFERENT GENRES THAT EXIST IN THE LIST OF MOVIES AND PUT THEM IN A TABLE CALLED allGenres
...
model_1_rmse
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie Effect Model", rmse = model_1_rmse)))
...
model_2_rmse
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie + User Effect Model", rmse = model_2_rmse)))
...
model_6_rmse
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie Effect Model with regularisation", rmse = model_6_rmse)))
...
model_3_rmse
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie + User Effect Model with regularisation", rmse = model_3_rmse)))

# Identify the winning parameters for all calculations to follow
mu <- mean(edx$rating)
...
# NOW MODEL RESIDUALS INDEPENDENTLY ON TOP OF THE PREVIOUS MODELS USING rpart ON THE NEW GENRE COLUMNS CREATED BEFORE
...
# Calcuate predicted ratings using mu + b_i + b_u + residuals, i.e. adding residuals to previous best estimate
...
model_4_rmse
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie + User Effect with regularisation + rpart residuals fit", rmse = model_4_rmse)))
...
results <- suppressWarnings(bind_rows(results, data_frame(method="Movie + User Effect + rpart residuals fit", rmse = model_5_rmse)))
